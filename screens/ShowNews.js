...
const ShowNews = ({route, navigation}) => {
  ...
  const getNews = async () => {
    const response = await fetch(
      'https://fakenewsdetectorapi.herokuapp.com/getnews?language=' + language,
    );
  ...
  const submitNews = async vote => {
    setIsLoading(true);
    const response = await fetch(
      'https://fakenewsdetectorapi.herokuapp.com/vote?id=' +
        news.id +
        '&vote=' +
        vote,
    );
  ...
  return (
    <View style={styles.container}>
      <View>
        <Text style={{fontSize: 20, fontWeight: 'bold', color: '#273469'}}>
          MESSAGE
        </Text>
      </View>
      {isLoading ? (
